...
void loop() {
  if (Serial.available()>0){                        // Envoi des données
    while(Serial.available()>0){
      Donnees_a_envoyer = Serial.read();
      Port_Bluetooth.write(Donnees_a_envoyer);      // Envoi sous forme de char
      
      Donnees_recues += Donnees_a_envoyer;          // On recompose les données envoyées
      delay(100);
      }
    Serial.print("> Ordinateur_1 : ");
    Serial.println(Donnees_recues);                  // retour de ce qui a été tapé
    }
  if (Port_Bluetooth.available()>0){                // Réception des données
